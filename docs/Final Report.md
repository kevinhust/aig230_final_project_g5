...
**RAG Pipeline (`src/rag_pipeline.py` — 355 lines)** *[AI-generated with human design and debugging]*
...
**Sentiment Analysis (`src/sentiment.py` — 302 lines)** *[Human-written]*
...
**Vector Store Builder (`src/vector_store.py` — 42 lines)** *[AI-generated]*
...
**Amazon KB Generator (`src/gen_amazon_kb.py` — 274 lines)** *[AI-generated, human-curated categories and keywords]*
...
**Evaluation Pipeline (`src/rag_eval.py` — 321 lines)** *[AI-generated with human metric design]*
...
**FastAPI Backend (`api.py` — 200 lines)** *[AI-generated]*
...
**Gradio Frontend (`app.py` — 177 lines)** *[AI-generated, human-customized]*
...
As required by the AIG230 course, here is our honest GenAI usage breakdown:

### 10.1 AI-Generated Components

The following components were primarily generated by Claude (Anthropic Claude Code), with the team providing design direction, requirements, and iterative refinement:

| Component | Tool Used | Human Role |
|-----------|-----------|------------|
| **RAG Pipeline** (`src/rag_pipeline.py`) | Claude Code | Architecture decisions, language/sentiment logic design, debugging and testing |
| **Amazon KB Generator** (`src/gen_amazon_kb.py`) | Claude Code | Dataset selection criteria, category/keyword curation, filtering thresholds |
| **FastAPI Backend** (`api.py`) | Claude Code | Endpoint design, response format specification |
| **Gradio Frontend** (`app.py`) | Claude Code | UI layout requirements, example query curation, CSS customization |
| **Evaluation Pipeline** (`src/rag_eval.py`) | Claude Code | Metric selection (what to measure), threshold decisions |
| **Vector Store Builder** (`src/vector_store.py`) | Claude Code | Chunk size/overlap parameter tuning |
| **Final Report** (`20Reports/Final Report.md`) | Claude Code + manual editing | Report structure, content review, results interpretation, tone adjustments |
| **24 Amazon FAQ files** (`kb/FAQ-*-*.md`) | `gen_amazon_kb.py` (AI-generated script) | Dataset filtering criteria, category definitions |

### 10.2 Human-Written Components

| Component | Details |
|-----------|---------|
| **16 Hand-crafted KB documents** | All 7 product FAQs, 6 policy documents, 2 code-switching guides, and Customer-Complaints.md were written entirely by the team |
| **Sentiment Module** (`src/sentiment.py`) | Multilingual keyword dictionaries, scoring logic, and threshold tuning — written and refined by the team |
| **Test Set** (`data/testset.csv`) | All 30 test questions and ground-truth answers were curated by the team |
| **Project architecture decisions** | Choice of BGE-M3 over other embeddings, RAG vs. fine-tuning decision, 4-language scope, code-switching detection approach |
| **Bug fixes and debugging** | Fixed language detection bug (`zh` → English mapping), ChromaDB stale collection issue, sentiment keyword whitespace issues |

### 10.3 Workflow

Our typical workflow was:

1. **Team decides** what to build next (e.g., "we need a sentiment analyzer for 4 languages")
2. **Claude generates** initial code based on our requirements
3. **Team reviews, tests, and modifies** the output — fixing bugs, adjusting parameters, adding features
4. **Repeat** until the component works correctly

For example, the sentiment module went through multiple iterations: the initial AI-generated version had issues with Spanish keyword formatting and threshold handling, which the team debugged and fixed manually.

### 10.4 Summary

| Category | Estimated AI % | Estimated Human % |
|----------|:--------------:|:-----------------:|
| Source code (all .py files) | ~70% | ~30% (design, debugging, testing, fixes) |
| Knowledge base (all kb/*.md) | ~55% (24 Amazon files) | ~45% (16 hand-crafted files) |
| Test data | 0% | 100% |
| Final Report | ~50% | ~50% (structure, review, results analysis) |
| **Overall project** | **~55%** | **~45%** |

We used **Claude Code (Anthropic)** as our primary AI tool. No other GenAI tools were used.
